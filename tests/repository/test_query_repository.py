...
class TestQueryRepository:
    ...
    async def test_create_and_retrieve_query(
        self, repository: QueryRepository, db_session: AsyncSession
    ):
    ...
    async def test_get_recent_queries(
        ...
        for i in range(10):
            await repository.create_query(question=f"Вопрос {i}", answer=f"Ответ {i}")
        await db_session.commit()
    ...
    async def test_queries_statistics(
        ...
        for question, answer, tokens, time in test_queries:
            await repository.create_query(
                question=question, answer=answer, tokens_used=tokens, response_time=time
            )
        await db_session.commit()
    ...
    async def test_delete_old_queries(
        ...
        for i in range(5):
            await repository.create_query(
                question=f"Старый вопрос {i}", answer=f"Старый ответ {i}"
            )
        await db_session.commit()
